...
#Database
#client is an object
client = pymongo.MongoClient("mongodb+srv://afafhe26552:RCL12345@cluster0.vtw847b.mongodb.net/" )
...
@app.route('/corona_records', methods=['GET'])
def get_all_patients():
    headers = {'Content-Type': 'application/json'}
    coronaData = corona_records.find()
    response = []
    # for patient in patients:
    for patientdata in coronaData:
        response.append({
        'id': str(patientdata['_id']),
        ...
        'patient_country':patientdata['patient_country'],
        'patient_city':patientdata['patient_city'],
        'patient_postal_address': patientdata['patient_postal_address'],
        'patient_email_address': patientdata['patient_email_address'],
        'patient_phone_number':patientdata['patient_phone_number'],
        'patient_covid_test':patientdata['patient_covid_test'],
        'fever_symptom': patientdata['fever_symptom'],
        'cold_symptom':patientdata['cold_symptom'],
        'cough_symptom ': patientdata['cough_symptom']
        })
    return jsonify(response), 200, headers
#############################################################################################################
